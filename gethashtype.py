...
while True:
    try:
        ...
        if first:
            ...
            h = simpledialog.askstring("Hash Identifier", "Enter the hash:")
        ...
        if len(jerar)==0:
            messagebox.showinfo("Hash Identification Result", "Not Found.")
        elif len(jerar)>2:
            jerar.sort()
            messagebox.showinfo("Hash Identification Result", "[+] Hash type: "+str(algorithms[jerar[0]]))
        else:
            jerar.sort()
            for a in range(len(jerar)):
                messagebox.showinfo("Other Hash Identification Result", "[+] "+str(algorithms[jerar[a]]))

        first = None
    except KeyboardInterrupt:
        messagebox.showinfo("Bye!", "Exiting...")
        root.destroy()  # Добавленная строка для закрытия окна
        break